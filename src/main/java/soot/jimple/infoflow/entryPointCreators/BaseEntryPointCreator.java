...
public abstract class BaseEntryPointCreator implements IEntryPointCreator {
	...
	protected final Set<SootClass> failedClasses = new HashSet<SootClass>();
	
	protected boolean substituteCallParams = false;
	protected List<String> substituteClasses;

	public boolean isAllowSelfReferences() {
		return allowSelfReferences;
	}
	 ...
	 */
	//FIXME: ugly hack here changed visability to override
	 ...
	 */
	protected Value buildArrayOfType(Body body, LocalGenerator gen, ArrayType tp,
	...
	public Type getSimpleTypeFromType(Type type) {
	...
	//FIXME: a cache for is compatible based on JProfiler
	private HashMap<SootClass,SootClass> isCompatibleCache = new HashMap<>(5000);
	...
	protected boolean isCompatible(SootClass actual, SootClass expected) {
		...
		SootClass cachedClass =isCompatibleCache.get(actual);
		if(cachedClass!=null && cachedClass.equals(expected)){
			return true;
		}
		while (true) {
			...
			if (expected.isInterface())
				for (SootClass intf : act.getInterfaces())
					if (intf.getName().equals(expected.getName())) {
						isCompatibleCache.put(actual,expected);
						return true;
					}
	 ...
	 * main method
	 * @param dummyClassName The name for the new class containing the dummy
	 * main method
	 */
